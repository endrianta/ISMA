...
<html lang="id">
    <head>
        <meta charset="UTF-8">
        <title>Kelola Banner - PT. ISMA</title>
    ...
    <body>
        <header><h1>Kelola Banner</h1></header>
        <div class="container">
            <a href="admin.php" class="back-link">← Kembali</a>
            <h2>Daftar Banner</h2>
            <a href="add_banner.php" class="btn-add">+ Tambah Banner</a>
            ...
            <table>
                ...
                <?php
                    $result = mysqli_query($conn,"SELECT * FROM banner ORDER BY id DESC");
                    while($row=mysqli_fetch_assoc($result)){
                        echo "<tr>
                        <td><img src='../assets/img/banner/".$row['image']."' height='60'></td>
                        <td class='action'>
                            <a href='edit_banner.php?id=".$row['id']."' class='edit'>Edit</a>
                            <a href='delete_banner.php?id=".$row['id']."' class='delete' onclick='return confirm(\"Hapus banner ini?\")'>Hapus</a>
                        </td>
                        </tr>";
                }
                ?>
