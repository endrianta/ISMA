...
// fungsi untuk perbaiki path gambar di konten Froala (pakai URL absolut)
function fixImagePath($html) {
    $baseUrl = "http://localhost/ISMA/"; // ganti sesuai hosting nanti
    return preg_replace_callback('/src="([^"]+)"/', function($matches) use ($baseUrl) {
        $path = str_replace('../', '', $matches[1]); // hilangkan ../
        $dirname = dirname($path);
        $basename = basename($path);
        return 'src="'.$baseUrl.$dirname.'/'.rawurlencode($basename).'"';
    }, $html);
}
...
if (isset($_POST['update'])) {
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        if (in_array($ext, $allowed_ext)) {
            ...
            $target = "../assets/img/news/" . $new_name;

            if (move_uploaded_file($file_tmp, $target)) {
                if (!empty($data['image']) && file_exists("../assets/img/news/".$data['image'])) {
                    unlink("../assets/img/news/".$data['image']);
                }
                $image = $new_name;
            }
        } else {
    ...
    if(mysqli_query($conn,$sql)){
        header("Location: edit_news.php?id=$id&success=1");
        exit();
    } else {
        echo "<p style='color:red;text-align:center;'>Gagal update berita!</p>";
    }
}
?>
...
<head>
  <meta charset="UTF-8">
  <title>Edit Berita - PT. ISMA</title>
...
<body>
  ...
  <div class="container">
    ...
    <form action="" method="POST" enctype="multipart/form-data">
      ...
      <textarea name="content" required><?php echo fixImagePath($data['content']); ?></textarea>

      <label>Penjelasan Rinci</label>
      <textarea name="detail"><?php echo fixImagePath($data['detail'] ?? ''); ?></textarea>
      ...
      <div class="preview">
          ...
          <?php if(!empty($data['image'])): ?>
              <img src="../assets/img/news/<?php echo htmlspecialchars($data['image']); ?>" alt="Gambar Berita">
          <?php else: ?>
